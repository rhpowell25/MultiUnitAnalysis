...
function [p_value] = TTest_SingleSession(Date)
...
%depth_change = mean(depth_noon) - mean(depth_morn);
[rank_depth, ~] = tiedrank([depth_morn; depth_noon]);
rank_depth_morn = rank_depth(1:length(depth_morn));
rank_depth_noon = rank_depth((length(depth_morn) + 1):end);
[~, p_value, ~, ~] = ttest2(depth_morn, depth_noon);
